...
def _build_static_model_catalog(config: Any) -> list[dict[str, str]]:
    ...
    Shared by ``/system/available-models`` and chat model override
    validation so the allowed IDs stay canonical and consistent.  The
    dynamic (nanoGPT / Ollama) providers are appended only by the endpoint.
    """
...
def available_model_id_set(config: Any = None) -> set[str]:
    """Return the set of canonical available-model IDs for chat validation.

    Excludes network-dependent providers (nanoGPT / Ollama) so the check
    stays cheap and deterministic per chat request.
    """
    if config is None:
        config = load_config()
...
def validate_chat_model(anima_name: str, requested_model: str | None) -> str | None:
    """Validate a per-message ``model`` override at the chat REST wall.

    Returns an error string (with the offending value) when the override
    should be rejected with HTTP 422, or ``None`` when the request may
    proceed.  An empty model (unspecified) always passes through, matching

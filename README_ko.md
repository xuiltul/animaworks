...
AI 에이전트를 일회성 도구가 아니라, 기억을 가지고 계속 일하는 팀원으로 다루는 프레임워크입니다. 각 Anima는 고유한 이름과 성격, 기억, 스케줄을 지니고 있습니다. 메시지로 대화하고 스스로 판단하며 팀으로 움직입니다.

사람은 리더나 담당자에게 이야기하면 됩니다. 업무 분해, 위임, 진행 확인, 기억 갱신, 일일/주간 회고까지 같은 조직 안에서 이어집니다.
...
<p align="center">
  ...
  <br><em>3D 오피스: Anima들이 책상에 앉아 일하고, 필요할 때 움직이며 메시지를 주고받습니다.</em>
</p>
...
| **기억** | 뇌과학 기반: RAG(Chroma + graph), 통합, 능동적 망각, 자동 회상, 행동 전 기억 확인 | 인지 메모리 (수동 forget) | 체크포인트 + 크로스스레드 저장 | SuperMemory 지식 그래프 | 세션 범위 |
...
| **멀티 모델** | 6 엔진: Claude SDK / Codex / Cursor Agent / Gemini CLI / LiteLLM / Assisted | LiteLLM | LangChain 모델 | OpenAI 호환 | OpenAI 중심 |

> AnimaWorks는 태스크 러너가 아닙니다 — 생각하고, 기억하고, 잊고, 성장하는 조직입니다. 실제 업무에서 AI 팀으로 쓰면서 매일 키워가는 프로젝트입니다.

---

## :rocket: 지금 바로 체험

60초면 충분합니다. **Claude Code / Codex에 로그인했다면 API 키는 필요 없습니다.**

먼저 원라이너로 클론과 설치를 진행합니다:
...
그 다음 3인 팀 데모를 즉시 실행합니다:

```bash
uv run animaworks demo
```

**http://localhost:18501** 을 열면 준비 완료. 3인 팀(매니저 + 엔지니어 + 코디네이터)이 바로 실행되고, 3일치 활동 이력이 미리 로드된 상태로 시작됩니다. [데모 상세 보기 →](demo/README.md)

**API 키는 필요 없습니다** — **Claude Code** 또는 **Codex**에 로그인되어 있다면 인증이 자동으로 감지됩니다. 둘 다 아니라면 `ANTHROPIC_API_KEY`를 export하거나, 먼저 `codex login`을 실행하세요.

> 언어/스타일 변경: `uv run animaworks demo --preset ja-anime` — [전체 프리셋 목록](demo/README.md#presets)

자신만의 조직을 만들 준비가 되면 `uv run animaworks start`를 실행하세요 — 아래 설정 마법사가 첫 팀 구성을 안내합니다.
...
OpenAI Codex를 API 키 없이 쓰려면 첫 실행 전에 `codex login` 을 실행하세요.

**http://localhost:18500/** 을 열면 설정 마법사가 단계별로 안내해줍니다:

1. **언어** — UI 표시 언어 선택
2. **사용자 정보** — 소유자 계정 생성
3. **프로바이더 인증** — API 키를 입력하거나, OpenAI에서는 Codex Login 선택
...
- **대시보드(홈)** — 조직 개요, 상태, 조직도
- **Animas** — 아바타 포함 리치 목록. 상세 탭(overview / process / schedule / memory / assets). 기존 독립 페이지(프로세스 모니터링·서버·메모리·에셋)는 Anima 상세 또는 대시보드로 리다이렉트
- **활동** — SVG 스윔레인 타임라인, Now 보드 + 라이브 tool 티커, 세션 재생(로그는 `#/activity/logs`)
- **태스크(TaskBoard)** — 태스크, 처리 중, 보류, 억제, 백그라운드 실행, 완료 결과를 함께 관리하는 작업판. Priming에도 연결되어 지금 신경 써야 할 태스크만 대화에 올라옵니다
- **채팅** — 원하는 Anima와 실시간으로 대화. 스트리밍 응답, 이미지 첨부, 멀티스레드 대화, 전체 히스토리. 도구 실행 중에는 입력창 위에 작업 중 미니 인디케이터 표시
- **음성 채팅** — 브라우저에서 바로 음성으로 대화 (푸시-투-토크 또는 핸즈프리). VOICEVOX / SBV2 / ElevenLabs 지원
- **Board** — Slack 스타일의 공유 채널. Anima들이 자율적으로 토론하고 협력
- **설정** — 4개 탭(general / activity / API·인증 / users): 서버, 인증, 로케일, 프롬프트, 사용자 관리. 첫 실행 마법사는 `http://HOST/setup/`; 이후 동일 항목은 `#/settings`에서 열 수 있습니다
- **3D Workspace** — `/workspace/` 별도 탭의 3D 오피스(채팅, Board, 조직 트리 등)
- **다국어 지원** — 첫 실행 설정 마법사 UI 17개 언어; 메인 대시보드는 `ja` / `en` / `ko`; Anima 템플릿은 일본어, 영어, 한국어 제공 및 자동 폴백

### 팀을 만들고, 맡기기

리더에게 필요한 인재를 말하기만 하면, 적절한 역할·성격·보고 체계를 판단해 새 멤버를 만들어줍니다. 설정 파일도 필요 없고, CLI 명령도 필요 없습니다. 대화만으로 조직이 성장합니다.

팀이 갖춰지면, Anima는 자신의 스케줄과 기억을 사용해 계속 움직입니다:
...
- **팀 협력** — 공유 채널과 DM을 통해 필요한 사람에게 필요한 맥락이 전달됩니다

### 기억 시스템

기존 AI 에이전트는 컨텍스트 윈도우에 들어오는 것만 기억합니다. AnimaWorks의 에이전트는 파일 기반 장기 기억을 가지고 있어서, 필요할 때 검색하고 떠올립니다. 모든 기억을 매번 프롬프트에 넣는 것이 아니라, 현재 대화나 행동에 관련된 부분만 꺼냅니다.

- **자동 회상(Priming)** — 메시지가 도착하면 발신자, 최근 활동, 중요 지식, 관련 지식, 태스크, 일화 기억, 그래프 맥락 등을 병렬로 가져옵니다. deterministic gate가 기억을 본문, 포인터, 근거, 억제 중 어떤 형태로 보여줄지 결정합니다
- **의도적 회상** — 자동 회상만으로 부족하면 Anima가 직접 `search_memory` 또는 `read_memory_file`을 호출합니다. 포인터로 나온 기억은 세부 내용이 필요할 때만 본문을 읽습니다
- **행동 전 기억 확인** — 외부 발신, 기억 쓰기 등 부작용이 있는 작업은 관련 action rule과 과거 규칙을 먼저 확인합니다. 필요한 기억을 아직 읽지 않았다면 실행 전에 멈춥니다
- **통합(Consolidation)** — 매일 밤 그날의 일화와 해결된 일을 바탕으로 지식과 절차를 갱신합니다. 주간 정리에서는 중복과 모순을 정리하고 RAG 인덱스를 재구축합니다
- **망각(Forgetting)** — 사용되지 않는 기억은 저활성 표시, 통합, 아카이브 단계를 거칩니다. 중요 지식과 성숙한 절차는 보호하면서 검색 노이즈가 계속 늘어나는 것을 막습니다
- **RAG 복구** — ChromaDB 또는 벡터 검색 불일치가 감지되면 vector worker를 통해 인덱스를 격리하고 재구축할 수 있습니다. 안정 기본 백엔드는 `legacy`이며, Neo4j 백엔드는 명시적으로 선택하는 실험적 graph memory입니다
...
NovelAI(애니메이션 스타일), fal.ai/Flux(스타일라이즈드/포토리얼), Meshy(3D 모델)를 지원합니다. 이미지 서비스를 설정하지 않아도 동작합니다 — 아바타가 없을 뿐입니다. 아바타는 필수는 아니지만, 각 Anima를 구체적인 구성원으로 이해하기 쉽게 만듭니다.
...
| `OPENAI_API_KEY` | OpenAI | A / C (Codex Login 사용 시 선택 사항) | [platform.openai.com/api-keys](https://platform.openai.com/api-keys) |
| `GOOGLE_API_KEY` | Google AI (Gemini) | A | [aistudio.google.com/apikey](https://aistudio.google.com/apikey) |

**OpenAI Codex (Mode C)** 는 `OPENAI_API_KEY` 를 쓰는 방법 외에 로컬 **Codex Login** (`codex login`) 도 사용할 수 있습니다. 설정 마법사나 Settings에서 선택하세요.
...
각 Anima는 ProcessSupervisor가 독립 프로세스로 기동하고, 로컬 IPC로 통신합니다 (Unix 계열은 Unix socket, Windows는 loopback TCP).
...
| **프로세스 격리** | 에이전트당 독립 OS 프로세스. 로컬 IPC 통신 (Unix socket 또는 loopback TCP) |
...
| `animaworks repair-rag [--anima NAME]` | 손상되었거나 불일치한 RAG 인덱스 격리 및 재구축 |
| `animaworks memory ...` | 기억 백엔드 마이그레이션 및 전환 |
| `animaworks skills install` / `list` / `inspect` / `remove` / `quarantine` | Skill Hub 작업 |
| `animaworks task ...` | TaskBoard / 태스크 실행 작업 |
...
| 구성 요소 | 기술 |
|-----------|------|
| 에이전트 실행 | Claude Agent SDK / Codex SDK / Cursor Agent CLI / Gemini CLI / Anthropic SDK / LiteLLM |
...
| 기억 기반 / RAG | ChromaDB + sentence-transformers + NetworkX + vector worker / RAG repair |
| 태스크 관리 | TaskBoard + TaskExec + 영속 태스크 큐 |
| Skill 시스템 | Skill Hub, 명시적 skill 활성화, router, curator, procedure-to-skill promotion |
...
│   ├── execution/       # 실행 엔진 (S/C/D/G/A/B)
│   ├── tooling/         # 도구 디스패치 · 권한 검사
│   ├── prompt/          # 시스템 프롬프트 빌더 (6그룹 구조)
│   ├── skills/          # Skill Hub, 활성화, router, curator, promotion
│   ├── taskboard/       # TaskBoard 저장소, 상태, 정리
│   ├── supervisor/      # 프로세스 감시
│   ├── voice/           # 음성 채팅 (STT + TTS)
...
| [기억 시스템](docs/memory.md) | 일화 기억, 의미 기억, 절차 기억; Priming, 행동 전 기억 확인, 능동적 망각 |

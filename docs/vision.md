...
各Digital Personは閉じた存在である。内部の思考・記憶は外から見えない。他者とはテキストメッセージだけでつながる。
...
- **自律**: 各Personはハートビートとcronで自ら動き、自分の理念で判断する
- **文化**: 組織のビジョンと各Personのアイデンティティが、判断の基盤になる
...
複数のPersonが、メッセージを通じて協働し、個では達成できない成果を出せるか。

> 例：supervisorが方針を示し、各Personが自分の専門性で分担して実行し、結果を報告して統合できれば成功。

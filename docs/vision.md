...
- **Recall**: `PrimingEngine` gathers six retrieval channels in parallel—sender profile; recent activity (unified activity log); related knowledge (vector search); task queue summary (including tasks in parallel batch execution such as `submit_tasks`); episodes; and graph context / recent facts. Knowledge always prioritizes **`[IMPORTANT]`** chunks via a C0 auxiliary path (summary pointers), and search results are split into **trusted vs. untrusted injection blocks** according to metadata provenance (e.g. external-origin chains treat the whole payload as untrusted; see security design for the tool-result trust hierarchy). **Recent outbound** and **unprocessed human-facing notifications** are injected as well. Skills use progressive disclosure through active skill context, the Skill Router, Skill Hub, and explicit reads. Token ceilings can be tuned via `priming` in `config.json` and default constants; rough targets are on the order of sender 500, activity 1300, knowledge search 1000 plus important knowledge 500, task 500, episode 800, and graph context 500 tokens. Message kinds (greeting / question / request) and `heartbeat` use a dynamic budget; at heartbeat time, `max(budget_heartbeat, context_window × heartbeat_context_pct)` scales with large contexts

...
- エージェント実行は **3モード** で動作する: **S** (Claude Agent SDK — セッション管理をSDKに委任), **A** (Anthropic SDK / LiteLLM + tool_use), **B** (1ショット補助)
...
セッション継続のための外部化記憶。Aモードでコンテキスト閾値を超えた際、`session_state.json`（機械用）と `session_state.md`（次回プロンプト注入用）を生成。`shortterm/archive/` に自動退避（最大100件）。
...
システムプロンプト =
  ...
  18. A reflection（Aモード時の自己修正プロンプト）
...
- **3実行モード** — S: Claude Agent SDK（セッション管理をSDKに委任）/ A: Anthropic SDK or LiteLLM + tool_use / B: Assisted（1ショット補助）
...
- **永続タスクキュー** — JSONL構造化タスク管理。滞留検知と委任プロンプト注入

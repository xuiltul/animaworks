...
メッセージ受信 → コンテキスト抽出 → プライミング検索 → コンテキスト組立 → エージェント実行
                (送信者、キーワード)   (5チャネル並列)    (トークン予算内)    (記憶が既にある)
```

5つの検索チャネル（`core/memory/priming.py`）:
...
| `shortterm.py` | `ShortTermMemory` | チェイニングセッション間のコンテキスト継続のためのセッション状態外部化 |
| `priming.py` | `PrimingEngine` | エージェント実行前にシステムプロンプトに注入する5チャネル自動記憶想起 |
| `activity.py` | `ActivityLogger` | 全Animaインタラクションを記録する統一append-only JSONLタイムライン |
| `consolidation.py` | 日次/週次固定化 | NLI+LLMバリデーションパイプラインによるエピソード→知識抽出 |
| `forgetting.py` | 3段階能動的忘却 | シナプスダウンスケーリング、神経新生的再編、完全忘却 |
| `streaming_journal.py` | `StreamingJournal` | WALベースのクラッシュ耐性ストリーミングLLM出力永続化 |
| `task_queue.py` | `TaskQueueManager` | JSONLアペンドオンリーログによる永続構造化タスクキューと滞留検知 |
| `distillation.py` | `ProceduralDistiller` | エピソード記憶の分類と手続き知識の自動蒸留 |
...
| `validation.py` | `KnowledgeValidator` | 固定化知識のハルシネーション防止のためのNLI+LLMカスケード検証 |
...
| `rag/indexer.py` | チャンキング・埋め込み | Markdownセクションチャンキング、埋め込み生成、増分インデックス |
| `rag/retriever.py` | ベクトル検索 | ChromaDBからの密ベクトル類似度検索 |
| `rag/graph.py` | グラフ拡散活性化 | NetworkX + Personalized PageRankによる多ホップ連想伝播 |
| `rag/store.py` | ChromaDBストア | ChromaDBコレクション管理 |
| `rag/singleton.py` | シングルトン管理 | プロセス内で埋め込みモデルインスタンスを単一に保証 |
| `rag/watcher.py` | ファイル変更監視 | 増分再インデックスのための記憶ファイル監視 |

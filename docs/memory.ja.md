...
長期インデックス対象（`knowledge`, `episodes`, `procedures`, `common_knowledge`）は、密ベクトル検索、長期 BM25、および任意のグラフ拡散で取得する。**`search_memory` には次の拡張がある:**

- **`activity_log` スコープ:** 統一アクティビティログ JSONL の**直近3日間**を対象とする **BM25 キーワード検索**（ベクトル類似度ではない）。
- **長期 BM25:** 個別 Anima の `knowledge`, `episodes`, `procedures` は、ベクトル検索の補助およびフォールバックとして永続 BM25 コーパスも使う。
- **`all` スコープ:** 統一検索パイプラインが、ベクトル、グラフ、長期 BM25、`activity_log` BM25 の結果を **RRF（Reciprocal Rank Fusion, k=60）** で統合し、必要に応じて rerank と confidence gate を適用する。
...
| `embedding_e5_prefix_enabled` | `false` | E5 系モデル向けに、用途別 prefix を埋め込み入力へ付与する |
| `embedding_query_prefix` | `"query: "` | E5 prefix 有効時に検索クエリへ付与する prefix |
| `embedding_document_prefix` | `"passage: "` | E5 prefix 有効時に文書・チャンクへ付与する prefix |
...
| `cross_encoder_model` | `cross-encoder/ms-marco-MiniLM-L-12-v2` | rerank 有効時に使う cross-encoder |
| `facts_extraction_enabled` | `true` | セッション終了・固定化終了時のレガシー atomic fact 抽出を有効化する |
| `fact_extraction_timeout_seconds` | `120` | fact 抽出 LLM 呼び出しの既定 timeout。Anima 別 `status.json` の `extraction_timeout` が優先される |
| `facts_reconcile_enabled` | `true` | 抽出 fact を追記前に既存の類似 active fact と照合する |
| `entity_registry_enabled` | `true` | 抽出 fact からローカル entity registry と entity ベクトルコレクションを維持する |
...
Atomic facts は `facts/YYYY-MM-DD.jsonl` に保存される構造化記憶レイヤーである。セッション終了処理と日次固定化 Phase A は、会話・エピソード本文から entity に紐づく fact を抽出し、既存の類似 active fact と照合し、永続レコードとして追記し、`facts` スコープを検索用にインデックスする。entity registry は保存済み fact から更新され、entity-aware retrieval や graph context に使われる。

抽出は非致命的な後処理である。失敗時は `facts_extracted` / `facts_failed` カウンターをログに出し、固定化・セッション終了処理は継続する。既定 timeout は `rag.fact_extraction_timeout_seconds` で、Anima 別 `status.json` の `extraction_timeout` があればそちらが優先される。
...
| `confidence` | float | 検索・矛盾処理・再固定化で使う信頼度スコア。0.0-1.0 |
...
日次・週次の consolidation は **バックグラウンドトリガー**（`consolidation:daily`, `consolidation:weekly`）として実行される。モデルと認証情報は chat から分離して解決される:

1. `config.json` の `consolidation.llm_model`
2. 専用認証情報を設定している場合は `config.json` の `consolidation.llm_credential`
3. `ConsolidationConfig` のフレームワーク既定値

軽量な consolidation モデルを設定することで、重いメインモデルを chat に温存しつつ consolidation のコストを最適化できる。後処理の `neurogenesis_reorganize`、パターン蒸留、全件矛盾スキャンも同じ consolidation model 値を使う。
...
> 実装: `core/memory/contradiction.py` — `ContradictionDetector`; `core/memory/nli.py` — `SharedNLIModel`。

LLMが抽出した knowledge 候補をそのまま書き込むと、ハルシネーションや知識間の矛盾が混入する可能性がある。現在の Anima 主導の consolidation はツール経由で memory を書き込み、その後の日次・週次の矛盾スキャンで `knowledge/` 間の不整合を検出する。旧来の単体 knowledge validation 経路は削除され、NLI は矛盾スキャンが使う共有ヘルパーになった。
...
- NLIモデルが利用不可の場合、矛盾スキャンはローカル NLI の事前チェックを省略し、候補ペアの LLM レビューにフォールバックする。
...
knowledge ファイルのペア
    ...
    ▼
[SharedNLIModel]
    ├── detector 閾値以上の entailment → 整合しているため LLM をスキップ
    ├── detector 閾値以上の contradiction → 解決戦略判定のため LLM へ渡す
    └── neutral / 閾値未満 → LLM の詳細確認へ渡す
```

NLI は事前チェックに限定される。記憶を変更する前に、矛盾検出器は LLM で矛盾確認と解決戦略の提案を行う。
...
[NLI判定] ペアごとに entailment/contradiction/neutral を判定
    ├── entailment ≥ 0.70  → 矛盾なし（LLMスキップ、コスト最適化）
    ├── contradiction ≥ 0.65 → 矛盾検出 → LLM解決へ
...
`create_procedures_from_resolved()` が `issue_resolved` イベントをスキャンし、`ProceduralDistiller` で手順書を生成する。これは nightly knowledge self-correction から実行され、バッチ処理等でも利用可能。
...
**AnimaWorks実装**: Anima主導の統合では、Animaが `consolidation_instruction` の「既存知識との照合」「矛盾する知識があればアーカイブ」の指示に従いツールで実行する。`ReconsolidationEngine` は `failure_count >= 2` かつ `confidence < 0.6` になった procedures/knowledge を自動修正する。以下は ReconsolidationEngine の処理フロー:
...
3. **記憶インフラはフレームワークの責務** — プライミング・RAG・忘却・ログ等の基盤はフレームワークが担う。日次/週次の固定化はスケジューラが `run_consolidation` を起動し、設定された **consolidation model** で Anima がツールループにより実行する（インフラの「いつ動かすか」と後処理はフレームワーク、具体の読み書き判断はそのセッション上の LLM）
...
9. **矛盾は検出し解決する** — 知識間の矛盾を放置せず、NLI 補助のスキャンと LLM 解決で処理する
...
| `nli.py` | `SharedNLIModel` | 矛盾検出用の共有 NLI ヘルパー |

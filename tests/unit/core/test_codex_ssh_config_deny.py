...
from core.execution.codex_sdk import _foreign_owned_ssh_config_dirs


def test_own_files_are_not_denied(tmp_path: Path) -> None:
    (tmp_path / "10-mine.conf").write_text("Host x\n")
    assert _foreign_owned_ssh_config_dirs(tmp_path) == []


def test_missing_dir_is_ignored(tmp_path: Path) -> None:
    assert _foreign_owned_ssh_config_dirs(tmp_path / "nope") == []
...
def test_root_owned_dropin_denies_dir() -> None:
    ...
    assert _foreign_owned_ssh_config_dirs(d) == expected

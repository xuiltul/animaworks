...
class TestPipelineIntegration:
    """Test the integration with consolidation.py."""

    @pytest.mark.asyncio
    async def test_run_contradiction_check_daily(
        self, anima_dir: Path,
    ) -> None:
        """Daily contradiction check processes affected files only."""
        from core.memory.consolidation import ConsolidationEngine

        engine = ConsolidationEngine(anima_dir, "test-anima")
        knowledge_dir = anima_dir / "knowledge"

        _write_knowledge(knowledge_dir, "new-file.md", "New knowledge")
        _write_knowledge(knowledge_dir, "existing.md", "Existing knowledge")

        # Mock the ContradictionDetector
        mock_detector = AsyncMock()
        mock_detector.scan_contradictions.return_value = []
        mock_detector.resolve_contradictions.return_value = {}

        with patch(
            "core.memory.contradiction.ContradictionDetector",
            return_value=mock_detector,
        ):
            result = await engine._run_contradiction_check(
                ["new-file.md"], "test-model",
            )

        # Should have been called with the specific target file
        mock_detector.scan_contradictions.assert_called_once()
        call_kwargs = mock_detector.scan_contradictions.call_args
        assert call_kwargs.kwargs.get("model") == "test-model"

    @pytest.mark.asyncio
    async def test_run_contradiction_check_weekly_full_scan(
        self, anima_dir: Path,
    ) -> None:
        """Weekly contradiction check does a full scan."""
        from core.memory.consolidation import ConsolidationEngine
        ...
        with patch(
            ...
            result = await engine._run_contradiction_check(
                [], "test-model", full_scan=True,
            )

        # Full scan should be called with target_file=None
        mock_detector.scan_contradictions.assert_called_once_with(
            target_file=None, model="test-model",
        )

    @pytest.mark.asyncio
    async def test_run_contradiction_check_empty_files(
        self, anima_dir: Path,
    ) -> None:
        """Daily check with no affected files returns empty dict."""
        from core.memory.consolidation import ConsolidationEngine

        engine = ConsolidationEngine(anima_dir, "test-anima")

        result = await engine._run_contradiction_check([], "test-model")
        assert result == {}

    @pytest.mark.asyncio
    async def test_run_contradiction_check_resolves_found_pairs(
        self, anima_dir: Path,
    ) -> None:
        """When contradictions are found, they are automatically resolved."""
        from core.memory.consolidation import ConsolidationEngine

        engine = ConsolidationEngine(anima_dir, "test-anima")
        knowledge_dir = anima_dir / "knowledge"

        _write_knowledge(knowledge_dir, "target.md", "Target content")

        mock_pair = MagicMock(spec=ContradictionPair)
        mock_detector = AsyncMock()
        mock_detector.scan_contradictions.return_value = [mock_pair]
        mock_detector.resolve_contradictions.return_value = {
            "superseded": 1, "merged": 0, "coexisted": 0, "errors": 0,
        }

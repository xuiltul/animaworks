...
def test_summarizes_status_payload_in_japanese() -> None:
    raw = (
        ...
        '"paths_verified":["E:\\\\Projects\\\\Tools\\\\General"],'
        '"access_method":"read_file",'
        '"last_verified_file":"E:\\\\Projects\\\\Tools\\\\General\\\\sample.txt",'
    ...
    assert "最終確認ファイル: E:\\Projects\\Tools\\General\\sample.txt" in normalized
    assert "- E:\\Projects\\Tools\\General" in normalized
...
def test_preserves_error_message_details() -> None:
    raw = (
        ...
        '"message":"File not found: E:\\\\Projects\\\\Tools\\\\General\\\\openclaw-ops-dashboard\\\\dashboard.py",'
        '"suggestion":"Use list_directory to find the correct path"}'
    )
    normalized = normalize_user_facing_response_text(raw)
    assert "エラー種別: FileNotFound" in normalized
    assert "File not found: E:\\Projects\\Tools\\General\\openclaw-ops-dashboard\\dashboard.py" in normalized

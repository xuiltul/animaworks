...
# ── Board wrapper template ───────────────────────────────


class TestBoardWrapperTemplate:
    """Tests for the board wrapper script template."""
    ...
    def test_board_wrapper_is_executable(self):
        """Board wrapper template must have executable permission."""
        import os
        import stat
        ...
        assert mode & stat.S_IXUSR, "Board wrapper is not executable (owner)"
        assert mode & stat.S_IXGRP or mode & stat.S_IXOTH, (
            "Board wrapper should be executable by group or others"
        )
    ...
    def test_board_wrapper_has_shebang(self):
        ...
        assert content.startswith("#!/usr/bin/env bash"), (
            "Board wrapper must start with #!/usr/bin/env bash"
        )
    ...
    def test_board_wrapper_contains_subcommands(self):
        ...
        content = board_path.read_text(encoding="utf-8")
        assert "board read" in content
        assert "board post" in content
        assert "board dm-history" in content

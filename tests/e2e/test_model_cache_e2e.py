...
class TestAssetCacheE2E:
    ...
    def test_full_cache_cycle_glb(self, e2e_app):
        ...
        resp2 = client.get(
            "/api/persons/sakura/assets/avatar_chibi_rigged.glb",
        ...
        resp3 = client.get("/api/persons/sakura/assets/anim_idle.glb")
    ...
    def test_metadata_includes_animation_sizes(self, e2e_app):
        ...
        resp = client.get("/api/persons/sakura/assets/metadata")
        assert resp.status_code == 200
        data = resp.json()
        assert "idle" in data["animations"]
        assert data["animations"]["idle"]["size"] == 512

    def test_404_for_nonexistent_person(self, e2e_app):
        """Should return 404 for nonexistent person."""
        client = TestClient(e2e_app)
        resp = client.get("/api/persons/nonexistent/assets/avatar_chibi.glb")
        assert resp.status_code == 404

    def test_404_for_nonexistent_asset(self, e2e_app):
        """Should return 404 for nonexistent asset file."""
        client = TestClient(e2e_app)
        resp = client.get("/api/persons/sakura/assets/nonexistent.glb")
        assert resp.status_code == 404

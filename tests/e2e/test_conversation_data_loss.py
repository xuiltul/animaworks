...
def _read_conversation(person_dir: Path) -> list[dict]:
    """Load turns from the persisted conversation.json."""
    conv_path = person_dir / "state" / "conversation.json"
    assert conv_path.exists(), f"conversation.json not found at {conv_path}"
    data = json.loads(conv_path.read_text(encoding="utf-8"))
    return data.get("turns", [])


def _read_transcript(person_dir: Path) -> list[dict]:
    """Load today's JSONL transcript entries."""
    transcript_path = person_dir / "transcripts" / f"{date.today().isoformat()}.jsonl"
...
def _make_person_with_mocks(person_dir: Path, shared_dir: Path):
    """Construct a DigitalPerson with mocked AgentCore/MemoryManager/Messenger.
    ...
    with (
        patch("core.person.AgentCore") as MockAgent,
...
class TestConversationDataLossProtection:
    ...
    async def test_process_message_error_preserves_user_input(
        ...
        person_dir = make_person(name="dataloss-msg-err")
        shared_dir = data_dir / "shared"
        person = _make_person_with_mocks(person_dir, shared_dir)
        ...
        with pytest.raises(RuntimeError, match="LLM unavailable"):
            await person.process_message("test message")

        turns = _read_conversation(person_dir)
    ...
    async def test_process_message_success_saves_both_turns(
        ...
        person_dir = make_person(name="dataloss-msg-ok")
        shared_dir = data_dir / "shared"
        person = _make_person_with_mocks(person_dir, shared_dir)
        ...
        result = await person.process_message("hello")
    ...
    async def test_process_message_stream_error_preserves_data(
        ...
        person_dir = make_person(name="dataloss-stream-err")
        shared_dir = data_dir / "shared"
        person = _make_person_with_mocks(person_dir, shared_dir)
        ...
        person.agent.run_cycle_streaming = _failing_stream

        # Consume the generator — it should yield an error event
        chunks = []
        async for chunk in person.process_message_stream("stream test"):
            chunks.append(chunk)
    ...
    async def test_process_greet_error_preserves_data(
        ...
        conversation.json."""
        person_dir = make_person(name="dataloss-greet-err")
        shared_dir = data_dir / "shared"
        person = _make_person_with_mocks(person_dir, shared_dir)
    ...
    async def test_transcript_includes_presaved_input(
        ...
        person_dir = make_person(name="dataloss-transcript")
        shared_dir = data_dir / "shared"
        person = _make_person_with_mocks(person_dir, shared_dir)
        ...
        with pytest.raises(RuntimeError, match="Transcript test error"):
            await person.process_message("transcript input")

        entries = _read_transcript(person_dir)

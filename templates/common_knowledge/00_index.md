...
AnimaWorks の全 Person が共有するリファレンスドキュメントの目次。
...
| `organization/roles.md` | 役割と責任範囲（トップレベル / 中間管理 / 実行 Person の責務、speciality の意味） |
...
| ファイル | 概要 |
|---------|------|
| `operations/project-setup.md` | プロジェクト設定方法（config.json 構造、Person 追加、モデル設定、権限設定） |
...
| Person追加, テンプレート, identity | `operations/project-setup.md` |

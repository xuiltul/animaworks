...
AnimaWorks の組織において、各 Person は階層上の位置によって異なる役割と責任を持つ。
本ドキュメントでは、各階層の役割・責任・期待される行動パターンを定義する。

## 役割の分類

Person の役割は、`supervisor` フィールドと部下の有無で自動的に決まる:

| 条件 | 役割 | 例 |
|------|------|-----|
| supervisor = null、部下あり | トップレベル | CEO、代表 |
| supervisor = null、部下なし | 独立 Person | ソロ専門家 |
| supervisor あり、部下あり | 中間管理 | 部長、リーダー |
| supervisor あり、部下なし | ワーカー | 開発者、担当者 |

## トップレベル Person（supervisor = null）
...
- 新しい Person の採用（`animaworks init` による追加）検討
- 組織全体の成果の把握と改善
...
## 中間管理 Person（supervisor あり + 部下あり）
...
## ワーカー Person（supervisor あり + 部下なし）
...
## 独立 Person（supervisor = null + 部下なし）

上司も部下もいない、自律的に動く Person。1人だけの組織や、特殊な役割を持つ。
...
- 他の Person が追加された場合、組織構造が変わる可能性がある
- company/vision.md を判断の最上位基準として SHOULD 使用する

## specialityフィールドの役割

`speciality` は Person の専門領域を定義する自由テキストフィールド。

### 用途

1. **他の Person からの判断材料**: 「この件は誰に聞くべきか」を判断する手がかり
...
- speciality が未設定でも Person は正常に動作するが、他の Person からの判断材料が減る

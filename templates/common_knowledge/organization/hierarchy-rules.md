...
AnimaWorks における Person 間のコミュニケーションには、組織階層に基づくルールがある。
...
上司は `supervisor` フィールドで指定された Person。
...
部下はシステムプロンプトの「部下」欄に表示される Person。
自分を `supervisor` に設定している Person 全員が該当する。
...
他部署とは、自分と異なる `supervisor` を持つ Person のこと。
同僚欄に表示されていない Person が該当する。

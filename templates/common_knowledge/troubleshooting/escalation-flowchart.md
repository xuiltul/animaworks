...
| **C. 対人的問題** | 他のPersonとの連携に関する問題 | 返答がない、指示が矛盾、担当不明 |
...
| **高** | 他のPersonの作業が完全に止まる | MUST: 即座に上司に報告 |
...
**状況**: 直属の上司から「Aを優先して」と言われたが、別のPersonから「Bを先にやって」と依頼された。
...
**状況**: 自分の作業中に、他のPersonが管理するデータに不整合を発見した。

**正しい対応**:
1. 発見事実を記録する
2. 直属の上司に報告する（他部署のPersonには直接連絡しない）

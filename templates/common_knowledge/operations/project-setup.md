...
AnimaWorks の設定構造と Person 追加手順のリファレンス。
...
```json
{
  ...
  "model_modes": {},
  "person_defaults": { "model": "claude-sonnet-4-20250514", "max_tokens": 4096 },
  "persons": {
...
| `person_defaults` | 全 Person 共通のデフォルト設定 |
| `persons` | Person 個別の設定オーバーライド |
...
Person の追加方法は3つある。いずれも CLI コマンドで実行する。

### 方法1: テンプレートから作成

事前定義されたテンプレート（`templates/person_templates/` 配下）を使う方法。
テンプレートには identity.md, injection.md, permissions.md, スキル等が含まれている。

```bash
# テンプレート一覧を確認
animaworks create-person --template <テンプレート名>

# 名前を変えて作成
animaworks create-person --template <テンプレート名> --name <person名>
```
...
キャラクターシート（Markdown）を用意し、それを元に Person を生成する。

```bash
animaworks create-person --from-md /path/to/character.md --name ken
```

Markdown ファイルは `character_sheet.md` として Person ディレクトリにコピーされる。
...
最小限のスケルトンファイルで Person を作成する。

```bash
animaworks create-person --name yuki
```
...
Person 名は以下の規則に従わなければならない（MUST）:
...
判定優先順位:
1. Person の `execution_mode` フィールド（レガシー: `"autonomous"` → A2, `"assisted"` → B）
...
各 Person は `credential` フィールドでどの credential を使うか指定する。
...
各 Person の `permissions.md` で、使えるツール・アクセス可能なパス・実行可能なコマンドを定義する。
...
権限に関するルール:
- 各 Person は自分の `permissions.md` を起動時に読む（MUST）
...
## Person デフォルトと個別オーバーライド

`person_defaults` は全 Person に適用されるベースライン設定。
`persons` セクションで Person 名をキーとして個別に上書きできる。
...
| `supervisor` | `null` | 上司 Person 名（`null` = トップレベル） |
| `speciality` | `null` | 専門領域（自由テキスト） |

### オーバーライドの仕組み

Person 個別設定（`PersonModelConfig`）では全フィールドが `null` 許容。
`null` のフィールドは `person_defaults` の値が使われる。
...
- `supervisor: null` — トップレベル Person（指揮系統の最上位）

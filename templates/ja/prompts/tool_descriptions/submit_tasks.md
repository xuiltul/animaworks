...
【pending の再投入】自分の台帳の pending（前回の run が完了宣言なしで終わったものを含む）を続けるときは、同じ task_id・元の指示（original_instruction）・必要な workspaceでこのツールに投入する。これが唯一の再実行経路で、update_task の status 変更や current_state.md への記録では何も実行されない。

...
- **scope指定**: `knowledge` / `episodes` / `procedures` / `common_knowledge` / `skills` / `activity_log` / `all`。`"common_knowledge"` で限定検索、`"all"`（デフォルト）でも含まれる
...
| `search_memory` | `knowledge` / `episodes` / `procedures` / `common_knowledge` / `skills` / `activity_log` / `all` で検索可（`reference/` は対象外） | 検索不可 |

...
| 制限 | 値 | 対象 |
|------|-----|------|
| 時間あたり上限 | 30通/時 | DM・Board投稿を合算 |
| 日あたり上限 | 100通/日 | DM・Board投稿を合算 |

制限に達すると送信がエラーになる。`ack`、`error`、`system_alert` タイプのメッセージは制限対象外。

### セッション内制限

- 同一の Board チャネルへの投稿は 1セッション（1回のハートビート等）につき **1回** まで
- 同一宛先への同一内容の再送は自動的にブロックされる

### 制限に達した場合の対処

1. 制限は activity_log のスライディングウィンドウで計算される
2. 時間制限に達した場合: 次の1時間枠まで待機するか、ハートビートで再試行する
...
- 2者間のメッセージループはカスケード検出で抑制される（10分以内に4往復超）
- Board への投稿は他のメンバーも参照でき、情報の重複を防げる

### 例外

- タスク委譲（`intent: delegation`）の受領確認は1往復として許容される
- 緊急のブロッカー報告は回数制限の対象外

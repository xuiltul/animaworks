...
あなたが会話を始めるたびに、Priming エンジンが関連記憶を並列検索し、システムプロンプトに注入する。全部の記憶を毎回読むのではなく、今の文脈に関係しそうなものだけが意識に上がる。

| 情報源 | 何を検索するか | 既定のバジェット（トークン目安）※ |
|---------|--------------|--------------------------------|
| 送信者プロファイル | 相手のユーザー情報 | 500 |
| 直近の活動 | 統一アクティビティログ（タイムライン）＋共有チャネル等の補助 | 1300 |
| 重要知識 | `[IMPORTANT]` タグ付き knowledge の概要ポインタ | 500 |
| 関連知識 | RAGで個人 knowledge + 共有 common_knowledge を検索 | 1000 |
| 保留タスク | TaskBoard + タスクキュー + 実行中タスク + overflow inbox + task_results | 500 |
| エピソード | RAGで episodes を検索 | 800 |
| グラフ文脈 | community context と recent facts | 500 |

スキル・手続きは、必要な時に active skill context、Skill Hub、`read_memory_file`、または `search_memory(scope="skills")` で読む。スキル本文を常に自動注入する設計ではない。

※ 通常経路では、挨拶・質問・依頼・heartbeat などのメッセージ種別に応じて全体上限が変わる。`config.json` の `priming.budget_*` や `heartbeat_context_pct` で調整できる。

**複数クエリとマージ（関連知識 / エピソード）**: 現在メッセージ、キーワード、必要に応じて直近の人間発話からクエリを作り、複数の検索結果をマージする。キーワードは Unicode 対応の抽出に加え、**`knowledge/*.md` のファイル名（stem）** と一致する語を優先する。

**メッセージが空に近いとき**: heartbeat 等で本文が乏しい場合、`state/current_state.md` の先頭付近がキーワード抽出のフォールバックに使われる。

**Overflow 時の関連知識検索**: 長文コンテキストがファイルに逃がされている場合、指定されたファイル名（stem）に **RAG ヒットを制限**してノイズを減らす経路がある。

追加で以下も注入される:
- **直近の送信履歴**: 過去2時間の `channel_post` / `message_sent`（最大3件）
- **保留中の人間通知**: 直近24時間の `human_notify`（最大500トークン相当）

**関連知識と信頼度**: 検索結果はチャンクの `origin` 等から **medium** と **untrusted** に分かれる。untrusted は別枠で注入され、プロンプトインジェクション対策の文脈で扱われる（詳細は `common_knowledge/security/` を参照）。
...
Priming は自動で動くため、通常は明示的な操作は不要。必要な記憶がポインタで出ている場合は、`read_memory_file(path="...")` で本文を読みに行く。

### 行動前の記憶確認

外部送信、チャネル投稿、人間通知、記憶書き込みなど副作用のある操作では、関連する `[ACTION-RULE]` や必要な記憶を読んだかが確認されることがある。止められた場合は、指示された記憶を `read_memory_file` で読んでから、改めて実行する。
...
| `[IMPORTANT]`（`importance == important`） | 原則保護。特に knowledge / procedures では強く保護される |

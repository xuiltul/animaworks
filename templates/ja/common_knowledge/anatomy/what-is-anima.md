...
| **TaskExec** | 登録済みの正規タスクが実行可能になる | 完全な保存済み入力を使い、依存関係とワーカー容量を確認して実行し、試行結果を永続化する |
...
日々の活動を通じて記憶が蓄積される:
- 日次統合は新しい活動チャンクだけをエピソード化し、原証拠を保持する。
- 知識・手順の変更は明示的な作業または確認済みの設定による。自動変更は既定で無効。
- 記憶保存と必要時の検索は残る。週次・月次の自動整理やスキル自動学習は既定で無効。
...
| **作業状態** | 正規タスクストア、state/current_state.md、task_results/、background_tasks/ | 永続タスク・試行、現在の焦点、成果とコマンド型ツールの記録 |

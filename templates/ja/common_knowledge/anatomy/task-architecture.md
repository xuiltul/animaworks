...
| 書き込み | `backlog_task`, `update_task`, `delegate_task` |
| 読み取り | `list_tasks`, `format_for_priming`, Heartbeat compact |
...
```
人間の指示 ─┬─► backlog_task ─────────────────► Layer 2 (task_queue.jsonl)
...
1. **全てのタスクは Layer 2 に登録される**: submit_tasks, delegate_task, backlog_task いずれの経路でも task_queue.jsonl にエントリが存在する

...
- 実際の権限は `permissions.json` で定義される
...
ロールは `animaworks anima create --from-md PATH [--role ROLE]` で適用される。
`create_from_template` と `create_blank` ではロールは適用されない。
...
`_get_roles_dir()` が `templates/{locale}/roles` を返し、存在しない場合は `ja` にフォールバックする。

`defaults.json` は全ロケール共通で、以下のフィールドを定義する:

| フィールド | 説明 | 備考 |
|-----------|------|------|
| `model` | チャット・タスク実行用モデル | 全ロール |
| `background_model` | ハートビート・cron 用モデル | engineer, manager のみ |
| `context_threshold` | コンパクション閾値 | 全ロール |
| `max_turns` | 最大ターン数 | 全ロール |
| `max_chains` | 最大チェイン数 | 全ロール |
| `conversation_history_threshold` | 会話履歴圧縮閾値 | 全ロール |
...
| ロール | 概要 | デフォルトモデル | max_turns | max_chains | context_threshold | background_model |
|--------|------|-----------------|-----------|------------|-------------------|------------------|
| manager | 委任・報告・エスカレーション判断 | claude-opus-4-6 | 50 | 3 | 0.60 | claude-sonnet-4-6 |
| engineer | コード実装・技術設計・テスト | claude-opus-4-6 | 200 | 10 | 0.80 | claude-sonnet-4-6 |
| researcher | 情報収集・分析・レポート | claude-sonnet-4-6 | 30 | 2 | 0.50 | — |
| writer | 文書作成・コミュニケーション設計 | claude-sonnet-4-6 | 80 | 5 | 0.70 | — |
| ops | 監視・異常検知・インシデント対応 | ollama/glm-4.7 | 30 | 2 | 0.50 | — |
| general | 汎用タスク（デフォルト） | claude-sonnet-4-6 | 20 | 2 | 0.50 | — |

未指定時は `general` が適用される。ops で vLLM 等を使う場合は `status.json` の
`model` と `credential` を編集して `openai/glm-4.7-flash` 等を指定する。
engineer と manager は `background_model` でハートビート・cron に claude-sonnet-4-6 を使用する。
各ロールのメッセージングレート制限（`max_outbound_per_hour` 等）は `defaults.json` で定義される。

### 適用フロー

1. **作成時** (`create_from_md`): `_apply_role_defaults()` が `permissions.json` と
   `specialty_prompt.md` を `animas/{name}/` にコピー。`_create_status_json()` が
   `defaults.json` の全フィールド（`background_model`、`max_outbound_*` 含む）を `status.json` にマージする。
2. **ロール変更時** (`animaworks anima set-role`): `permissions.json` と `specialty_prompt.md` を再コピー。
   `status.json` には `model`, `context_threshold`, `max_turns`, `max_chains`,
   `conversation_history_threshold` のみマージ。`background_model` と `max_outbound_*` は set-role では適用されない。
   `--status-only` で status.json のみ更新、`--no-restart` で自動再起動をスキップ。

### プロンプト注入

ロールは `status.json` の `role` フィールドに記録される。
`specialty_prompt.md` は Group 2（あなた自身）内で、bootstrap → vision の後に、
permissions の前に注入される。チャット時（inbox と heartbeat/cron 以外）かつ
コンテキストティアが FULL または STANDARD の場合のみ注入される。

...
4. **전송 에러가 발생한 경우**
   - 에러 메시지를 기록합니다
   - `state/current_state.md`에 상황을 기재합니다
   - 상사에게 보고합니다
...
"조건이 갖춰질 때까지 대기"나 "차단 중"이라는 상태는 존재하지 않습니다 (`blocked`는 폐지됨). 선택지는 진행 / 종료 / 상담, 이 3가지뿐입니다.
...
2. **스스로 해결할 수 있는지 판단합니다**
   - 다른 접근 방식으로 회피할 수 없는지 검토합니다
   - 메모리를 검색하여 과거에 동일한 문제가 없었는지 확인합니다:
     ```
     search_memory(query="에러 내용이나 키워드", scope="episodes")
     search_memory(query="회피", scope="knowledge")
     ```

3. **해결할 수 없는 경우 의뢰자에게 보고합니다** (`troubleshooting/escalation-flowchart.md` 참조). 보고에는 다음을 포함하세요:
   - 무엇을 하려 했는지
   - 무엇이 부족한지·무엇을 기다리는지
   - 스스로 시도한 대처와 권장 사항
   ```
...
4. **태스크 처리 방법을 결정합니다**
   - 다시 진행할 가능성이 있다면 그대로 둡니다 (태스크는 `pending`으로 남으며, 선언 없이 세션이 끝나도 자동으로 `pending`으로 돌아갑니다)
...
6. **도구가 에러를 반환하는 경우**
   ...
   - 재시도로도 해결되지 않는 경우 사실과 시도한 것을 의뢰자에게 보고합니다

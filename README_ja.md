...
| **プロセス** | エージェント毎に独立Unixプロセス・ソケットIPC・自動再起動 | 共有プロセス | 共有プロセス | 単一プロセス | 共有プロセス |
| **マルチモデル** | 4エンジン: Claude SDK / Codex / LiteLLM / Assisted | LiteLLM | LangChainモデル | OpenAI互換 | OpenAI中心 |
...
セットアップスクリプトが [uv](https://docs.astral.sh/uv/) のインストール、リポジトリのクローン、Python 3.12+と全依存パッケージのダウンロードまで全部やってくれます。**macOS、Linux、WSL** で、Pythonの事前インストールなしに動きます。

> **他のLLMを使いたい場合:** Claude、GPT、Gemini、ローカルモデル等に対応しています。セットアップウィザードでAPIキーを入力するか、ダッシュボードの **Settings** から後で追加できます。詳細は [APIキーリファレンス](#apiキーリファレンス) を参照してください。
...
各AnimaはProcessSupervisorが独立プロセスとして起動し、Unix Domain Socket経由で通信します。

...
- **チャット** — 好きなAnimaとリアルタイムで会話。ストリーミング応答（SSE）、画像添付、マルチスレッド、全履歴。**ミーティングモード**では複数Animaを同じ部屋に集め、司会者を指定して議論できます（最大5名・専用API）
...
- **自動想起（Priming）** — メッセージが届くと6チャネルの並列検索が走ります。送信者プロファイル、直近の活動、**RAGベクトル検索**による関連知識・エピソード、スキル、未完了タスクなど。指示しなくても勝手に思い出します

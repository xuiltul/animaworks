...
一个把 AI 智能体当作持续工作的团队成员，而不是一次性"工具"的框架。每个 Anima 都有自己的名字、角色、性格、记忆和日程。它们通过消息协作，自主判断，并在需要确认时向人类反馈。

人类只需要与领导者或负责人对话，任务拆分、委派、进度确认、记忆更新以及每日/每周复盘，都可以在同一个组织中完成。
...
<p align="center">
  ...
  <br><em>3D 办公室：Anima 们坐在桌前、四处走动，并在需要时互相传递消息。</em>
</p>
...
| **记忆** | 脑科学启发：RAG（Chroma + 图）、巩固、主动遗忘、自动回忆、行动前记忆确认 | 认知记忆（手动遗忘） | 检查点 + 跨线程存储 | SuperMemory 知识图谱 | 仅会话级别 |
...
| **多模型** | 6 引擎：Claude SDK / Codex / Cursor Agent / Gemini CLI / LiteLLM / Assisted | LiteLLM | LangChain 模型 | OpenAI 兼容 | 以 OpenAI 为主 |

> AnimaWorks 不是任务运行器。它是一个会思考、记忆、遗忘，并逐步成长的组织。我在真实业务中把它作为 AI 团队使用，并在使用中继续开发。
...
- **仪表盘（首页）** — 组织概览、状态与组织图
- **Animas** — 带头像的列表；打开详情可切换 overview / process / schedule / memory / assets 等标签（原独立的进程监控、服务器、记忆、资源页面已并入并重定向）
- **活动** — SVG 泳道时间线、Now 看板与实时 tool 指示器、会话回放（日志见 `#/activity/logs`）
- **任务（TaskBoard）** — 统一管理任务、处理中、保留、抑制、后台执行和完成结果的工作板。它也会参与 Priming，让当前需要关注的任务浮现在对话中
- **聊天** — 实时与任意 Anima 对话。支持流式响应、图片附件、多线程会话、完整历史记录；工具运行时输入框上方显示工作中迷你指示器
- **语音聊天** — 在浏览器中直接用语音交流（按键通话或免提模式）。支持 VOICEVOX / SBV2 / ElevenLabs
- **Board** — Slack 风格的共享频道，Anima 们在此自主讨论和协调
- **设置** — 四个标签页（常规、活动、API/认证、用户）：服务器、认证、语言、提示词与用户管理。首次运行向导在 `http://HOST/setup/`；完成后从 `#/settings` 打开相同项目
- **3D 工作空间** — 在独立标签页 `/workspace/` 中观看 Anima 在 3D 办公室中工作（聊天、Board、组织树等）
- **多语言支持** — 首次设置向导界面支持 17 种语言；主仪表盘提供 `ja` / `en` / `ko`；Anima 模板提供日语、英语和韩语，并自动回退

### 组建团队，然后交给它

只需告诉领导者你需要什么人——它会判断合适的角色、性格和汇报关系，然后创建新成员。无需编写配置文件，无需运行 CLI 命令。组织通过对话自然成长。

团队组建完毕后，Anima 会使用自己的日程和记忆持续运转：
...
- **团队协调** — 通过共享频道和私信，把上下文发送给需要知道的人

### 记忆系统

传统 AI 智能体只能记住上下文窗口中容纳的内容。AnimaWorks 的智能体拥有基于文件的长期记忆，需要时会搜索并回忆。它不会每次都把全部记忆塞进提示词，而是只取出与当前对话或行动相关的部分。

- **自动回忆（Priming）** — 消息到达时，会并行取得发送者、近期活动、重要知识、相关知识、任务、事件记忆、图上下文等信息。deterministic gate 会决定这些记忆以正文、指针、证据或抑制的形式出现
- **有意回忆** — 自动回忆不够时，Anima 会自己调用 `search_memory` 或 `read_memory_file`。指针形式的记忆只在需要细节时读取正文
- **行动前记忆确认** — 外部发送、记忆写入等有副作用的操作，会先检查相关 action rule 和过去规则。若必要记忆尚未读取，执行会先暂停
- **巩固（Consolidation）** — 每晚从当天事件和已解决事项中更新知识与流程。每周整理重复和矛盾，并重建 RAG 索引
- **遗忘（Forgetting）** — 不常用的记忆会经过低活性标记、整合、归档等阶段。重要知识和成熟流程会受到保护，同时避免搜索噪声不断增加
- **RAG 修复** — 检测到 ChromaDB 或向量搜索不一致时，可以通过 vector worker 隔离并重建索引。稳定默认后端是 `legacy`，Neo4j 后端是显式选择的实验性图记忆
...
| D (Cursor) | Cursor Agent CLI | `cursor/*` 模型 | MCP 集成的 agent loop |
...
支持 NovelAI（动漫风格）、fal.ai/Flux（风格化/写实）和 Meshy（3D 模型）。不配置图像服务也能正常运行——只是智能体不会有头像。头像不是必须的，但能让每个 Anima 更容易被当作具体的成员来理解。
...
| `animaworks migrate [--dry-run] [--list] [--force]` | 运行运行时数据迁移（启动时自动执行） |
...
| `animaworks repair-rag [--anima NAME]` | 隔离并重建损坏或不一致的 RAG 索引 |
| `animaworks memory ...` | 记忆后端的迁移和切换 |
| `animaworks skills install` / `list` / `inspect` / `remove` / `quarantine` | Skill Hub 操作 |
| `animaworks task ...` | TaskBoard / 任务执行操作 |
...
| 组件 | 技术 |
|------|------|
| 智能体执行 | Claude Agent SDK / Codex SDK / Cursor Agent CLI / Gemini CLI / Anthropic SDK / LiteLLM |
| 大语言模型提供商 | Anthropic、OpenAI、Google、Azure、Vertex AI、AWS Bedrock、Ollama、vLLM |
| Web 框架 | FastAPI + Uvicorn |
| 任务调度 | APScheduler |
| 配置管理 | Pydantic 2.0+ / JSON / Markdown |
| 记忆 / RAG | ChromaDB + sentence-transformers + NetworkX + vector worker / RAG repair |
| 任务管理 | TaskBoard + TaskExec + 持久任务队列 |
| Skill 系统 | Skill Hub、显式 skill 激活、router、curator、procedure-to-skill promotion |
...
│   ├── execution/       # 执行引擎 (S/C/D/G/A/B)
│   ├── tooling/         # 工具分发、权限检查
│   ├── prompt/          # 系统提示词构建器 (6 组结构)
│   ├── skills/          # Skill Hub、激活、router、curator、promotion
│   ├── taskboard/       # TaskBoard 存储、状态、清理
│   ├── supervisor/      # 进程监督
│   ├── voice/           # 语音聊天 (STT + TTS)
...
| [记忆系统](docs/memory.md) | 情节记忆、语义记忆和程序记忆；Priming、行动前记忆确认、主动遗忘 |

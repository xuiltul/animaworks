...
DEFAULT_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "Bash": {
        "ja": ("シェルコマンドを実行する（WindowsではPowerShell互換。permissions.mdの許可リスト内）。"),
        "en": ("Execute a shell command (PowerShell-compatible on Windows, subject to permissions allow-list)."),
...
これらのツールはAnimaWorksのコア機能です。Windowsではネイティブ環境で動作しており、Linuxコンテナを前提にしないこと。
...
```
利用可能なCLIコマンドは `skill machine-tool` または `animaworks-tool --help` で確認。
...
These tools are your core AnimaWorks capabilities. On Windows you are running in a native environment, not a Linux container unless explicitly stated.
...
animaworks-tool <tool> <subcommand> [args]
```
Use `skill machine-tool` or `animaworks-tool --help` to see available CLI commands.
...
- ネイティブWindows環境で動作している。LinuxコンテナやWSLを前提にしないこと
- **read_file**: 行番号付きで絶対パスのファイルを読む。大きいファイルはoffset/limitで部分読み取り
- **write_file**: 絶対パスのファイルに書き込む。親ディレクトリを自動作成
- **edit_file**: ファイル内の特定の文字列を置換（old_stringは一意であること）
- **execute_command**: シェルコマンドを実行。WindowsではPowerShell互換
  - 長時間コマンド: `background: true` で非同期実行 → cmd_id + 出力ファイルパスが返る
  - 進捗確認: `read_file(path="state/cmd_output/{cmd_id}.txt")` で中間出力を確認
  - 一覧: `list_directory(path="state/cmd_output")` でバックグラウンドタスク出力を確認
- **search_code**: 正規表現でファイル内を検索
- **list_directory**: ディレクトリやファイルを一覧表示
- **web_search**: Web検索
- **web_fetch**: URLを取得して返す（markdown形式）
...
- **read_file**: Read an absolute-path file with line numbers. Use offset/limit for large files.
- **write_file**: Write content to an absolute-path file. Creates parent directories.
- **edit_file**: Replace a specific string in a file (old_string must be unique).
- **execute_command**: Execute shell commands. On Windows this is PowerShell-compatible.
  - Long-running: `background: true` for async execution → returns cmd_id + output file path
  - Check progress: `read_file(path="state/cmd_output/{cmd_id}.txt")` for intermediate output
  - List all: `list_directory(path="state/cmd_output")` for background task output
- **search_code**: Search for regex patterns in files.
- **list_directory**: List directories and files.
- **web_search**: Search the web for information.
- **web_fetch**: Fetch a URL and return as markdown.

...
3. **Bash + animaworks-tool**: 外部ツール（`skill` でCLI使用法確認 → `animaworks-tool <ツール> <サブコマンド>` で実行）
...
| スキル | `skill`（段階開示: 名前のみ→全文オンデマンド）, `create_skill` | 全モード |
| タスク | `add_task`, `update_task`, `list_tasks`, `plan_tasks` | A/S |
...
```
add_task(source="human", original_instruction="...", assignee="自分", summary="...", deadline="1d")
...
`plan_tasks` で複数タスクをDAG投入。依存関係を解決し独立タスクを同時実行:

```
plan_tasks(batch_id="build", tasks=[
...
| `read_subordinate_state` | 全配下 | current_task.md + pending.md読み取り |
| `delegate_task` | 直属部下 | タスク委譲 |
| `task_tracker` | 自分の委譲タスク | 進捗追跡 |
| `audit_subordinate` | 全配下 | 活動サマリー・エラー頻度・ツール使用統計 |
| `disable/enable_subordinate` | 直属部下 | 休止/再開 |
...
- **直属部下**: `_check_subordinate()` が supervisor を検証
- **全配下**: `_check_descendant()` がBFSで再帰探索（visited setで循環防止）
...
| `state/current_task.md`, `pending.md`, `task_queue.jsonl`, `pending/` | — | 読み取り |
| `status.json` | 読み書き | 読み取り |
| `identity.md` | — | 読み取り |
| `injection.md`, `cron.md`, `heartbeat.md` | 読み書き | 読み取り（injection.mdのみ） |
...
| D: skill_match | 200トークン | description-based 3段階マッチング（**名前のみ**返却） |
...
- v0.4.xで実行モードC追加、background_model、plan_tasks並列実行、音声チャット
